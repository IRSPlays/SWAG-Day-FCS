...
export const content = {
  ...
  title: "THE GAMES",
  serif: "four events. one champion. zero mercy.",
  events: [
    ...
    <SlideShell>
      ...
      <div className="absolute left-16 top-[10%] z-10">
        ...
        <h2 className="mt-4 font-display uppercase leading-none">
          <LetterStagger text={c.title} delay={0.12} className="block text-[170px] text-ice" />
        </h2>
        <ClipWipeReveal delay={0.5} from="left">
          <p className="mt-3 font-serifit text-[44px] italic text-vio">{c.serif}</p>
        </ClipWipeReveal>
      </div>

      <div className="absolute inset-x-16 bottom-16 z-10 flex flex-col">
        {c.events.map((e, i) => (
          <div key={e.n} className="flex items-center gap-10 border-b-2 border-ice/10 py-5 last:border-b-0">
